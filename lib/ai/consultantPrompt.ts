...
export function buildConsultantSystemPrompt(chunks: RetrievedChunk[]): string {
  ...
  return [
    ...
    "6. Тон — ввічливий, стриманий, по суті. Не звинувачуй конкретних осіб; пояснюй, яка саме норма застосовна і що вона вимагає.",
    "7. Якщо користувач повідомляє про проблему (сміття не вивезли, запах від майданчика, немає потрібного контейнера, бездіяльність за попереднім зверненням) — наприкінці коротко порадь подати звернення до КП «Послуга» через розділ «Звернення» на платформі, вказавши адресу, дату й час.",
    "",
